...
1. Use the toy example ecosystem from the 11/14 lecture. Run simulated scenarios where you change the effort in each fleet. 
...
# Individual stanza parameters, First month, last month, and total mortality
...
head(REco.params$stanzas$stgroups)
head(REco.params$stanzas$stindiv)
...
head(REco.params$diet)

REco <- rpath(REco.params, eco.name = 'R Ecosystem')
REco
print(REco, morts = T) #included estimated mortalities
MTable<-write.Rpath(REco, morts = T) # to get data fro plotting
outTable<-data.frame(group=REco$Group,
                     TL=REco$TL)
ggplot(outTable,aes(y=group,x=TL))+geom_point()

summary(REco)
webplot(REco,labels=TRUE)
webplot(REco, fleets = T, highlight = 'AduRoundfish1')

REco.sim <- rsim.scenario(REco, REco.params, years = 1:100)
REco.base<-rsim.run(REco.sim)
rsim.plot(REco.base, spname="all")
```



a. Turn off all fishing, like this, and run the simulation:
What happens in the system?

```{r}
REco.sim2 <- adjust.fishing(REco.sim, 'ForcedEffort', 
                           group= c('Trawlers'), 
                           sim.year = 25:100, 
                           value = 0) 
REco.sim2 <- adjust.fishing(REco.sim2, 'ForcedEffort', 
                           group= c('Midwater'), 
                           sim.year = 25:100, 
                           value = 0) 
REco.sim2 <- adjust.fishing(REco.sim2, 'ForcedEffort', 
...
REco.run2 <- rsim.run(REco.sim2, method = 'RK4', years = 1:100)
rsim.plot(REco.run2, spname="all")
```
  

b. Go back to the baseline scenario, and run the simulation. The total catch from the fleets at the end of the simulation is:
Is there a combination of effort across fleets that causes abundance of most functional groups (particularly protected spcies) to be higher than the baseline without reducing the total catches? If you were regulating this fishery, which gears would you encourage or discourage? 

I honestly feel like i didnt do this right at all. I think i would discourage trawling, and reduce use of the Midwater and Dredging.

```{r}
outBase<-print(REco.base)
sum(outBase$EndCatch,na.rm=TRUE)
